public class _06_Max_Element_Update {
    ...
    public static void updateUtil(int stidx,int si,int sj,int idx,int newValue){
        if(si>idx || sj<idx){
            return;
        }

        if(si==sj){
            segmentTree[stidx] = newValue;
        }

        if(si!=sj){ //Non Leaf
            segmentTree[stidx] = Math.max(segmentTree[stidx],newValue);
            int mid = (si+sj)/2;
            updateUtil(stidx*2+1, si, mid, idx, newValue);
            updateUtil(stidx*2+2, mid+1, sj, idx, newValue);
        }
    }
